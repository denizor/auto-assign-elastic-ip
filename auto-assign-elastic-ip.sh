...
aws_get_instance_environment() {
	instance_environment=$(aws ec2 describe-tags --region $instance_region --filters "Name=resource-id,Values=$1" "Name=key,Values=aegro:environment" --query "Tags[*].Value" --output text)
	if [ -n "$instance_environment" ]; then return 0; else return 1; fi
}

aws_get_unassigned_eips() {
	local describe_addreses_response=$(aws ec2 describe-addresses --region $instance_region --filters "Name=tag:aegro:environment,Values=$instance_environment" --query "Addresses[?AssociationId==null].AllocationId" --output text)
